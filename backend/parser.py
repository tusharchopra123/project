...
import pandas as pd
import re
from backend.isin_lookup import get_scheme_name as lookup_isin_name
                ...
                for line in lines:
                    ...
                    if line_lower.startswith("total"):
                        ...
                        if len(numbers) >= 2:
                             ...
                             # Assumption: Cost is 1st or 2nd?
                             # In "Total Cost Value", Cost is usually first.
                             # But let's check values.
                             # 2.63M vs 2.83M.
                             # Usually Cost < Value in this bull market.
                             # But strictly, CAMS format is usually Cost then Value.
                             # Let's verify with the Dump from Step 431: "Total 2,636... 2,834..."
                    ...
                    if date_match and "stamp duty" not in line_lower and "charges" not in line_lower:
                        ...
                        if numbers:
                            ...
                            # Heuristic: Find the Amount column (usually not the NAV/Units)
                            # If multiple numbers, Amount is usually large?
                            # Or we rely on column position?
                            # For simplified logic, assume First Big Number or just First Number?
                            # Step 579: "4,999.75 129.64 38.56 ..." (Amount, NAV, Units)
                            # So Index 0 is often Amount.
                            amount = clean_nums[0]
                    ...
                    # (Fallback if we didn't match transaction logic and line has numbers)
                    # Only if we HAVEN'T extracted transactions for this fund? 
                    # Combining them is tricky. 
                    # If the PDF is "Holdings Only", there are no Dates at start.
